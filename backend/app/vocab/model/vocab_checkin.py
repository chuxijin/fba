...
class VocabCheckin(Base):
    """打卡记录表"""

    __tablename__ = 'vocab_checkin'
    __table_args__ = (
        sa.UniqueConstraint('user_id', 'checkin_date', name='uq_vocab_checkin_user_date'),
        {'comment': '打卡记录表'},
    )
    ...
    new_words: Mapped[int] = mapped_column(default=0, comment='当日新学单词数')
    review_words: Mapped[int] = mapped_column(default=0, comment='当日复习单词数')
