...
async def create_session(
    ...
    session = await session_service.get_session_detail(
        db=db, session_id=new_session.id, user_id=current_user.user_id
    )
...
async def submit_session(
    ...
    result = await session_service.submit_session(
        db=db, session_id=pk, user_id=current_user.user_id, obj=obj,
    )
...
async def upsert_records(
    ...
    obj.session_id = pk
    result = await session_service.upsert_records(db=db, user_id=current_user.user_id, obj=obj)
    return response_base.success(data=result)
