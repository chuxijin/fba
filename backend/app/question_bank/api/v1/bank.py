...
async def get_bank_questions_all(
    ...
    return response_base.success(data={
        'total': bank.get('q_count_cache', len(result_with_answer)) if isinstance(bank, dict) else 0,
        'offset': offset,
        'limit': limit,
...
async def get_bank_list(
    ...
    """🌍 公开接口 - 任何人都可以查看题库树形列表"""
    data = await bank_service.get_list(db=db, cat_id=cat_id, status=status, scope=scope, keyword=keyword, bank_type=bank_type, parent_id=parent_id)
    return response_base.success(data=data)
