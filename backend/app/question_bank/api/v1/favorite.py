...
@router.get('/questions/{question_id}/check', summary='检查是否已收藏', name='qbank_favorite_check')
async def check_favorited(
    ...
    """检查用户是否已收藏指定题目"""
    is_favorited = await question_favorite_dao.check_favorited(
        db=db, user_id=current_user.user_id, question_id=question_id
    )
    return response_base.success(data=is_favorited)
