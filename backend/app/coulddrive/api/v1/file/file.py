...
async def get_file_list(
    ...
    try:
        service = CouldDriveService.create_from_request(
            db=db, request=request, x_token=x_token, drive_type=params.drive_type, drive_account_id=drive_account_id
        )

        file_list = await service.get_disk_list(params=params, db=db, drive_account_id=drive_account_id)
...
async def get_share_file_list(
    ...
    try:
        ...
        file_list = await service.get_share_list(params=params, db=db, drive_account_id=drive_account_id)
...
async def batch_rename_files(
    ...
    try:
        ...
        # 初始化进度信息
        file_operate_service_instance.update_progress(
            temp_task_id,
            {
                'type': 'start',
                'message': '开始批量重命名...',
        ...
        # 标记任务完成
        file_operate_service_instance.update_progress(
            temp_task_id, {'type': 'complete', 'message': '批量重命名完成', 'stats': result}
        )
        return response_base.success(data=result)
