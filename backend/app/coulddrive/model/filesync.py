...
class SyncConfig(Base, UserMixin):
    ...
    exclude_template_id: Mapped[int | None] = mapped_column(
        BigInteger,
        ForeignKey('rule_template.id', ondelete='SET NULL', use_alter=True),
        nullable=True,
        comment='排除规则模板ID',
        init=False,
    )
    rename_template_id: Mapped[int | None] = mapped_column(
        BigInteger,
        ForeignKey('rule_template.id', ondelete='SET NULL', use_alter=True),
        nullable=True,
        comment='重命名规则模板ID',
    ...
    # 关系
    drive_account: Mapped['DriveAccount'] = relationship('DriveAccount', back_populates='sync_configs', init=False)
    sync_tasks: Mapped[list['SyncTask']] = relationship(
        'SyncTask', back_populates='sync_config', cascade='all, delete-orphan', init=False
    )
    exclude_template: Mapped['RuleTemplate'] = relationship(
        'RuleTemplate', foreign_keys=[exclude_template_id], init=False
    )
    rename_template: Mapped['RuleTemplate'] = relationship(
        'RuleTemplate', foreign_keys=[rename_template_id], init=False
    )
...
class SyncTask(Base, UserMixin):
    ...
    config_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey('filesync_config.id', ondelete='CASCADE'), nullable=False, index=True, comment='配置ID'
    )
    ...
    # 关系
    sync_config: Mapped['SyncConfig'] = relationship('SyncConfig', back_populates='sync_tasks', init=False)
    task_items: Mapped[list['SyncTaskItem']] = relationship(
        'SyncTaskItem', back_populates='sync_task', cascade='all, delete-orphan', init=False
    )
...
class SyncTaskItem(Base):
    ...
    task_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey('filesync_task.id', ondelete='CASCADE'), nullable=False, index=True, comment='任务ID'
    )
    ...
    # 关系
    sync_task: Mapped['SyncTask'] = relationship('SyncTask', back_populates='task_items', init=False)
