...
class DriveAccount(Base, UserMixin):
    ...
    sync_configs: Mapped[list['SyncConfig']] = relationship(
        init=False, back_populates='drive_account', cascade='all, delete-orphan'
    )
    resources: Mapped[list['Resource']] = relationship(init=False, back_populates='user', cascade='all, delete-orphan')
