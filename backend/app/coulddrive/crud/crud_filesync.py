...
class CRUDSyncTask(CRUDPlus[SyncTask]):
    ...
    async def has_running_task(self, db: AsyncSession, *, config_id: int) -> bool:
        ...
        stmt = select(func.count(SyncTask.id)).where(
            and_(SyncTask.config_id == config_id, SyncTask.status == 'running')
        )
        result = await db.execute(stmt)
    ...
    async def get_tasks_by_config_id(
        self, db: AsyncSession, *, config_id: int, status: str | None = None
    ) -> list[SyncTask]:
        """
        根据配置ID获取同步任务列表
        ...
        stmt = (
            select(SyncTask)
            .options(noload(SyncTask.sync_config), noload(SyncTask.task_items))
    ...
    async def get_task_with_items(self, db: AsyncSession, *, task_id: int) -> SyncTask | None:
        ...
        stmt = select(SyncTask).options(joinedload(SyncTask.task_items)).where(SyncTask.id == task_id)
...
class CRUDSyncTaskItem(CRUDPlus[SyncTaskItem]):
    ...
    async def get_items_by_task_id(
        self, db: AsyncSession, *, task_id: int, status: str | None = None, operation_type: str | None = None
    ) -> list[SyncTaskItem]:
        """
        根据任务ID获取同步任务项列表
