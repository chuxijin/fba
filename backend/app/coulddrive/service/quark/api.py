...
class QuarkApi:
    ...
    async def get_account_info(self, **kwargs):
        ...
        params = {'fr': kwargs.pop('fr', 'pc'), 'platform': kwargs.pop('platform', 'pc')}
    ...
    async def list_files(
        self, pdir_fid: str = '0', page: int = 1, size: int = 50, sort: str = 'file_type:asc,file_name:asc', **kwargs
    ):
        """获取文件列表
    ...
    async def delete_files(
        self, file_ids: List[str], action_type: int = 2, exclude_fids: Optional[List[str]] = None, **kwargs
    ):
        """删除文件
        ...
        data = {'action_type': action_type, 'filelist': file_ids, 'exclude_fids': exclude_fids or []}

        resp = await self._request(Method.POST, url, params=params, data=data)
        return resp.json()

    async def query_task(self, task_id: str, retry_index: int = 0, **kwargs):
        """查询任务状态
    ...
    async def create_folder(
        self, pdir_fid: str, file_name: str, dir_path: str = '', dir_init_lock: bool = False, **kwargs
    ):
        """创建文件夹
        ...
        data = {'pdir_fid': pdir_fid, 'file_name': file_name, 'dir_path': dir_path, 'dir_init_lock': dir_init_lock}

        resp = await self._request(Method.POST, url, params=params, data=data)
        return resp.json()

    @assert_ok
    async def rename_file(self, fid: str, file_name: str, **kwargs):
        """重命名文件
        ...
        data = {'fid': fid, 'file_name': file_name}
    ...
    async def get_share_page(
        self, page: int = 1, size: int = 50, order_field: str = 'created_at', order_type: str = 'desc', **kwargs
    ):
        """获取分享列表
    ...
    @assert_ok
    async def cancel_shared(self, share_ids: List[str], **kwargs):
        """删除分享
    ...
    @assert_ok
    async def create_share(self, fid_list: List[str], title: str, url_type: int = 1, expired_type: int = 1, **kwargs):
        """创建分享
        ...
        data = {'fid_list': fid_list, 'title': title, 'url_type': url_type, 'expired_type': expired_type}

        resp = await self._request(Method.POST, url, params=params, data=data)
        return resp.json()

    @assert_ok
    async def get_share_password(self, share_id: str, **kwargs):
        """获取分享密码
    ...
    async def get_share_token(
        self, pwd_id: str, passcode: str = '', support_visit_limit_private_share: bool = True, **kwargs
    ):
        """获取分享token
    ...
    @assert_ok
    async def get_file_info_by_path(self, file_paths: List[str], **kwargs):
        """根据路径获取文件信息
