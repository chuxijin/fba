...
class ResourceService:
    ...
    @staticmethod
    async def get_list(*, db: AsyncSession, params: GetResourceListParam) -> dict[str, Any]:
        """
        获取资源列表
    ...
    @staticmethod
    async def create(
        *, db: AsyncSession, obj: CreateResourceParam, created_by: int, auto_vectorize: bool = False
    ) -> GetResourceDetail:
        """
        创建资源
        ...
        try:
            ...
            share_info_params = ListShareInfoParam(
                drive_type=DriveType(user.type), source_type='link', source_id=obj.url, page=1, size=1
            )
        ...
        if resource_data.get('pwd_id'):
            ...
            if existing_resource:
                ...
                if (
                    updated_resource.pwd_id
                    and 'view_count' in update_data
                    and update_data['view_count'] != existing_resource.view_count
                ):
                    try:
                        history_param = CreateResourceViewHistoryParam(
                            pwd_id=updated_resource.pwd_id, view_count=updated_resource.view_count
                        )
        ...
        if resource.pwd_id:
            try:
                history_param = CreateResourceViewHistoryParam(
                    pwd_id=resource.pwd_id, view_count=resource.view_count or 0
                )
                await resource_view_history_dao.create(db, history_param)
            except Exception:
                # 记录浏览量历史失败不影响资源创建
    ...
    @staticmethod
    async def update(
        *, db: AsyncSession, pk: int, obj: UpdateResourceParam, updated_by: int, auto_refresh: bool = False
    ) -> GetResourceDetail:
        """
        更新资源
        ...
        if auto_refresh and resource.url:
            try:
                ...
                if user:
                    ...
                    # 获取分享信息参数
                    share_info_params = ListShareInfoParam(
                        drive_type=DriveType(user.type), source_type='link', source_id=resource.url, page=1, size=1
                    )
        ...
        # 如果浏览量有变化且有pwd_id，记录浏览量历史
        if updated_resource.pwd_id and 'view_count' in update_data and update_data['view_count'] != resource.view_count:
            try:
                history_param = CreateResourceViewHistoryParam(
                    pwd_id=updated_resource.pwd_id, view_count=updated_resource.view_count
                )
    ...
    async def refresh_share_info(db: AsyncSession, resource_id: int, updated_by: int) -> GetResourceDetail:
        ...
        if updated_resource.pwd_id:
            try:
                history_param = CreateResourceViewHistoryParam(
                    pwd_id=updated_resource.pwd_id, view_count=updated_resource.view_count or 0
                )
                await resource_view_history_dao.create(db, history_param)
            except Exception:
                # 记录浏览量历史失败不影响分享信息刷新
    ...
    @staticmethod
    async def delete(*, db: AsyncSession, ids: list[int], deleted_by: int) -> int:
        """
        批量删除资源（软删除）
    ...
    async def update_view_count(*, db: AsyncSession, pwd_id: str, increment: int = 1) -> None:
        ...
        # 记录浏览量历史
        history_param = CreateResourceViewHistoryParam(pwd_id=pwd_id, view_count=resource.view_count + increment)
        await resource_view_history_dao.create(db, history_param)
    ...
    async def get_overall_statistics_trend(
        *, db: AsyncSession, params: GetOverallStatisticsTrendParam
    ) -> OverallStatisticsTrendResponse:
        """
        获取整体统计趋势
        ...
        if trend_data:
            summary = {
                'total_resources_growth': trend_data[-1].total_count - trend_data[0].total_count
                if len(trend_data) > 1
                else 0,
                'total_views_growth': trend_data[-1].total_views - trend_data[0].total_views
                if len(trend_data) > 1
                else 0,
        ...
        return OverallStatisticsTrendResponse(trend_data=trend_data, summary=summary)

    @staticmethod
    async def refresh_by_id(
        *, db: AsyncSession, pk: int, expired_type: int = 7, updated_by: int | None = None, set_permanent: bool = False
    ) -> Dict[str, Any]:
        """
        刷新资源分享链接
        ...
        try:
            ...
            resource = await resource_dao.get(db, pk)
            if not resource:
                return {'success': False, 'error': '资源不存在', 'resource_id': pk}

            # 检查资源状态
            if resource.is_deleted or resource.status != 1:
                return {'success': False, 'error': '资源已删除或停用', 'resource_id': pk}

            # 获取用户网盘账户信息
            drive_account = await drive_account_dao.get(db, resource.user_id)
            if not drive_account or not drive_account.is_valid:
                return {'success': False, 'error': '网盘账户不存在或无效', 'resource_id': pk}

            # 检查 cookies 是否存在
            if not drive_account.cookies:
                return {'success': False, 'error': '网盘账户缺少认证信息', 'resource_id': pk}

            # 检查是否有文件ID
            if not resource.file_id:
                return {'success': False, 'error': '缺少文件ID，无法重新分享', 'resource_id': pk}
            ...
            if new_share_info.pwd_id:
                try:
                    history_param = CreateResourceViewHistoryParam(pwd_id=new_share_info.pwd_id, view_count=0)
                    await resource_view_history_dao.create(db, history_param)
                except Exception:
                    # 记录浏览量历史失败不影响资源刷新
                    pass
            ...
            return {'success': False, 'error': str(e), 'resource_id': pk}

    @staticmethod
    async def refresh_to_permanent(
        *, db: AsyncSession, category_id: int, updated_by: int | None = None
    ) -> Dict[str, Any]:
        """
        刷新分类资源为永久链接
    ...
    async def refresh_expiring_resources(
        *,
        ...
        try:
            ...
            expired_resources = []
            if include_expired:
                expired_resources = await resource_dao.get_expired_resources(db, current_time=current_time)
            ...
            all_resources.sort(
                key=lambda x: (
                    x.expired_at or datetime.max.replace(tzinfo=timezone.tz_info),
                    getattr(x, 'expiry_category', ''),
                )
            )
            ...
            for resource in all_resources:
                try:
                    ...
                    refresh_result = await ResourceService.refresh_by_id(
                        db=db, pk=resource.id, expired_type=expired_type, updated_by=updated_by
                    )
    ...
    async def vector_search(
        *,
        ...
        results = await resource_dao.vector_search(
            db, query_text, limit, similarity_threshold, category_id=category_id, category_ids=category_ids
        )
...
class ResourceViewHistoryService:
    ...
    @staticmethod
    async def get_list(*, db: AsyncSession, params: GetResourceViewHistoryListParam) -> dict:
        """
        获取浏览量历史记录列表
    ...
    @staticmethod
    async def get_view_trend(*, db: AsyncSession, params: GetResourceViewTrendParam) -> ResourceViewTrendResponse:
        """
        获取资源浏览量趋势
        ...
        trend_data = [
            ResourceViewTrendData(record_time=record.record_time, view_count=record.view_count)
            for record in trend_records
        ]

        return ResourceViewTrendResponse(
            pwd_id=params.pwd_id, current_view_count=resource.view_count, trend_data=trend_data
        )

    @staticmethod
    async def update_view_count(*, db: AsyncSession, params: UpdateResourceViewCountParam) -> None:
        """
        更新资源浏览量并记录历史
        ...
        # 记录浏览量历史
        history_param = CreateResourceViewHistoryParam(pwd_id=params.pwd_id, view_count=params.view_count)
        await resource_view_history_dao.create(db, history_param)
