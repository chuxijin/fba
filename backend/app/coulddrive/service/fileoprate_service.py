...
class FileOperateService:
    ...
    async def batch_rename_files(
        ...
        **kwargs,
        ...
        await self._send_progress(
            progress_callback,
            f'遍历完成，找到 {len(filtered_items)} 个符合条件的项目',
            len(filtered_items),
            len(filtered_items),
        )
        ...
        await self._send_progress(
            progress_callback,
            f'筛选完成，{len(items_to_rename)} 个需要重命名',
            len(items_to_rename),
            len(items_to_rename),
        )
        ...
        await self._execute_renames(
            service, items_to_rename, rename_rules, task_id, db, account_key, stats, progress_callback
        )
    ...
    def _parse_rules(
        self, params: BatchRenameParam, template_data: Optional[Any], task_label: str, stats: Dict[str, Any]
    ...
    async def _collect_items(
        ...
        for file_info in params.file_infos:
            ...
            if params.recursive and file_info.is_folder:
                ...
                try:
                    folder_items = await self._list_recursive(service, file_info.file_id, file_info.file_path)
    ...
    async def _list_recursive(
        self, service: CouldDriveService, parent_id: str, parent_path: str
    ) -> List[Dict[str, Any]]:
        """递归列出目录下所有文件"""
        all_items = []
        ...
        try:
            ...
            for item in file_list:
                ...
                if item.is_folder:
                    ...
                    sub_items = await self._list_recursive(service, item.file_id, item.file_path)
    ...
    def _filter_by_scope(self, items: List[Dict], target_scope: str) -> List[Dict]:
        ...
        return [
            item
            for item in items
            if (target_scope == 'file' and not item.get('is_folder', False))
            or (target_scope == 'folder' and item.get('is_folder', False))
        ]

    def _classify_items(self, items: List[Dict], rules: List[RenameRule]) -> tuple[List[Dict], List[Dict]]:
        """分类需要重命名和跳过的文件"""
        to_rename = []
        to_skip = []
    ...
    async def _do_rename(
        ...
        stats['pending_task_items'].append(
            self._create_task_item(
                task_id,
                f'重命名{"成功" if status == "success" else "失败"}',
                original_path,
                new_path,
                original_name,
                item.get('file_size', 0),
