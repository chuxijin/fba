...
class GetCompareDetail(SchemaBase):
    ...
    to_add: list[dict[str, Any]] = Field(
        ..., description='需要添加的文件，包含source_item、target_full_path、target_parent_path等信息'
    )
...
class BatchRenameParam(SchemaBase):
    ...
    target_scope: Literal['file', 'folder', 'all'] = Field(
        'all', description='重命名目标范围 (file:仅文件, folder:仅文件夹, all:所有)'
    )
