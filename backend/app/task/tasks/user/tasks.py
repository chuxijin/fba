...
async def _refresh_all_valid_drive_users(task) -> dict[str, Any]:
    """刷新所有有效的网盘用户信息的异步实现"""
    result = {'checked_users': 0, 'refreshed_users': 0, 'failed_users': 0, 'skipped_users': 0, 'refresh_details': []}
    ...
    async with async_db_session() as db:
        ...
        for account in valid_accounts:
            try:
                ...
                # 构建用户信息查询参数
                user_info_params = UserInfoParam(drive_type=DriveType(account.type))
