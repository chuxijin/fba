...
class DashboardStats:
    ...
    def __init__(
        ...
        self.template_distribution = template_distribution or {}
...
async def get_dashboard_stats(db: CurrentSession) -> ResponseSchemaModel[dict]:
    ...
    template_rows = (
        await db.execute(
            ...
            .select_from(Subscription)
            .join(SubscriptionTemplate, SubscriptionTemplate.id == Subscription.template_id)
            .where(
                Subscription.status == 'active',
                Subscription.valid_period.contains(now),
            )
            .group_by(SubscriptionTemplate.name)
        )
    ).all()
    template_dist = {row[0]: row[1] for row in template_rows}
