WITH category_node AS (
    SELECT id FROM sys_category WHERE app_code = 'youanshang' AND type = 'knowledge_point' AND code = 'kp_ex_a_travel'
)
...
SELECT 'gongkao', '路程最值', 'kp-ex-a-travel', CAST($${"type": "doc", "content": [{"type": "heading", "attrs": {"level": 1}, "content": [{"type": "text", "text": "路程最值"}]}, {"type": "paragraph", "content": [{"type": "text", "text": "行程的极限相遇点。"}]}]}$$ AS jsonb), NULL, '路程最值的核心考点与实战推导。', category_node.id, CAST('["数量关系", "路程最值"]' AS jsonb), FALSE, TRUE, TRUE, NOW(), 0, 0, CAST('{"content_type": "knowledge_point", "category_code": "kp_ex_a_travel", "source": "fba_content_engine"}' AS jsonb), 1, NOW()
