...
class JobApplicationService:
    ...
    @staticmethod
    async def get(db: AsyncSession, _id: int, user_id: int) -> JobApplication | None:
        return await crud_job_application.get(db, _id, user_id=user_id)
    ...
    async def get_list(
        ...
        raw_data = await crud_job_application.get_list(db, user_id, job_posting_id, application_status)
    ...
    @staticmethod
    async def create(db: AsyncSession, obj_in: CreateJobApplication, user_id: int) -> JobApplication:
        """创建投递记录"""
        job_posting = await crud_job_posting.get(db, obj_in.job_posting_id)
        if not job_posting:
            raise errors.NotFoundError(msg='招聘信息不存在')
        job_application = await crud_job_application.create(db, obj_in, user_id)
        return job_application

    @staticmethod
    async def update(db: AsyncSession, obj_in: UpdateJobApplication, _id: int, user_id: int) -> JobApplication | None:
        return await crud_job_application.update(db, obj_in, _id, user_id=user_id)

    @staticmethod
    async def delete(db: AsyncSession, _id: int, user_id: int) -> int | None:
        return await crud_job_application.delete(db, _id, user_id=user_id)
