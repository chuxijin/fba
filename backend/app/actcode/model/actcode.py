...
class ActcodeBatch(Base):
    ...
    app_id: Mapped[str] = mapped_column(sa.String(32), index=True, comment='应用ID')
    ...
    reward_data: Mapped[dict] = mapped_column(sa.JSON, comment='权益数据JSON')
    generator_config: Mapped[dict | None] = mapped_column(sa.JSON, default=None, comment='生成器配置JSON')
...
class Actcode(Base):
    ...
    id: Mapped[id_key] = mapped_column(init=False)
    batch_id: Mapped[int] = mapped_column(sa.BigInteger, comment='批次ID')
...
class ActcodeUsage(Base):
    ...
    id: Mapped[id_key] = mapped_column(init=False)
    code_id: Mapped[int] = mapped_column(sa.BigInteger, index=True, comment='激活码ID')
    app_id: Mapped[str] = mapped_column(sa.String(32), index=True, comment='应用ID')
    user_id: Mapped[str] = mapped_column(sa.String(64), index=True, comment='用户ID')
    used_time: Mapped[datetime] = mapped_column(
        TimeZone, init=False, default_factory=timezone.now, comment='使用时间'
    )
    ip_address: Mapped[str | None] = mapped_column(sa.String(64), default=None, comment='IP地址')
