...
async def _fallback_external_search(
    query: str, final_limit: int, providers: list[str] | None = None
) -> List[dict[str, Any]]:
...
async def _save_quark_and_share(
    account_id: int, target_folder_id: str, share_url: str, ext_note: str | None = None
) -> dict[str, Any] | None:
    ...
    try:
        ...
        files_ext_info = [
            {'file_id': f.file_id, 'file_ext': f.file_ext if hasattr(f, 'file_ext') else {}} for f in files
        ]
...
async def _save_baidu_and_share(
    account_id: int,
...
async def _save_not_implemented(
    account_id: int, folder_id: str, share_url: str, ext_note: str | None
) -> dict[str, Any] | None:
    """占位：未实现的网盘类型"""
    return None
...
async def perform_resource_search(
    query: str, limit: int = 5, cloud_types: str | None = None, enable_external_search: bool = True
) -> list[dict[str, Any]]:
    """
    执行资源搜索的核心逻辑
...
def register_resource_search_tools(mcp: FastMCP) -> None:
    ...
    async def search_resources(
        query: str, limit: int = 5, cloud_types: str = None, enable_external_search: bool = True
    ) -> list[dict[str, Any]]:
        """
        搜索资源库
