...
async def get_mcp_config_detail(db: CurrentSession, config_id: int) -> ResponseSchemaModel[dict]:
    ...
    return response_base.success(data={"id": obj.id, "mcp": obj.mcp, "field": obj.field, "value": obj.value} if obj else None)
...
async def upsert_mcp_config_batch(
    ...
    return response_base.success(
        data=[{"id": i.id, "mcp": i.mcp, "field": i.field, "value": i.value} for i in results]
    )
