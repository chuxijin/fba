#!/usr/bin/env python3
# -*- coding: utf-8 -*-
from typing import Annotated

from fastapi import APIRouter, Path, Query, Request

from backend.app.admin.schema.message import (
    CreateMessageParam,
    DeleteMessageParam,
    GetMessageDetail,
    GetMyMessageItem,
    MessageQueryParam,
    UpdateMessageParam,
)
from backend.app.admin.service.message_service import message_service
from backend.common.pagination import DependsPagination, PageData
from backend.common.response.response_schema import ResponseModel, ResponseSchemaModel, response_base
from backend.common.security.jwt import DependsJwtAuth
from backend.database.db import CurrentSession, CurrentSessionTransaction

router = APIRouter()


# ------------------------------------------------------------------ 用户端（固定路径优先于参数化路径）


@router.get(
    '/mine',
    summary='获取我的消息列表',
    dependencies=[DependsJwtAuth, DependsPagination],
)
async def sys_messages_get_mine(
    request: Request,
    db: CurrentSession,
    unread_only: Annotated[bool, Query(description='是否只看未读')] = False,
    message_type: Annotated[str | None, Query(description='消息类型')] = None,
) -> ResponseSchemaModel[PageData[GetMyMessageItem]]:
    """获取当前用户的消息分页列表"""
    page_data = await message_service.get_my_list(
        db=db,
        user_id=request.user.id,
        unread_only=unread_only,
        message_type=message_type,
    )
    return response_base.success(data=page_data)


@router.get(
    '/mine/unread-count',
    summary='获取我的未读消息数',
    dependencies=[DependsJwtAuth],
)
async def sys_messages_get_unread_count(
    request: Request,
    db: CurrentSession,
) -> ResponseModel:
    """获取当前用户的未读消息数"""
    count = await message_service.count_unread(db=db, user_id=request.user.id)
    return response_base.success(data={'count': count})


@router.put(
    '/mine/read-all',
    summary='标记全部消息已读',
    dependencies=[DependsJwtAuth],
)
async def sys_messages_mark_all_read(
    request: Request,
    db: CurrentSessionTransaction,
) -> ResponseModel:
    """标记当前用户全部消息已读"""
    count = await message_service.mark_all_read(db=db, user_id=request.user.id)
    return response_base.success(data={'count': count})


@router.get(
    '/mine/{pk}',
    summary='获取我的消息详情',
    dependencies=[DependsJwtAuth],
)
async def sys_messages_get_mine_detail(
    request: Request,
    db: CurrentSession,
    pk: Annotated[int, Path(description='消息 ID')],
) -> ResponseSchemaModel[GetMyMessageItem]:
    """获取当前用户的消息详情"""
    detail = await message_service.get_my_detail(db=db, pk=pk, user_id=request.user.id)
    return response_base.success(data=detail)


@router.put(
    '/mine/{pk}/read',
    summary='标记消息已读',
    dependencies=[DependsJwtAuth],
)
async def sys_messages_mark_read(
    request: Request,
    db: CurrentSessionTransaction,
    pk: Annotated[int, Path(description='消息 ID')],
) -> ResponseModel:
    """标记单条消息已读"""
    await message_service.mark_read(db=db, pk=pk, user_id=request.user.id)
    return response_base.success()


# ------------------------------------------------------------------ 管理端


@router.get(
    '',
    summary='分页获取消息列表',
    dependencies=[
        DependsJwtAuth,
        DependsPagination,
    ],
)
async def get_message_list(
    db: CurrentSession,
    message_type: Annotated[str | None, Query(description='消息类型')] = None,
    target_type: Annotated[str | None, Query(description='目标类型')] = None,
    status: Annotated[int | None, Query(description='状态')] = None,
    keyword: Annotated[str | None, Query(description='标题或内容关键词')] = None,
    biz_source: Annotated[str | None, Query(description='来源模块')] = None,
) -> ResponseSchemaModel[PageData[GetMessageDetail]]:
    """
    获取消息分页列表

    :param db: 数据库会话
    :param message_type: 消息类型
    :param target_type: 目标类型
    :param status: 状态
    :param keyword: 标题或内容关键词
    :param biz_source: 来源模块
    :return:
    """
    params = MessageQueryParam(
        message_type=message_type,
        target_type=target_type,
        status=status,
        keyword=keyword,
        biz_source=biz_source,
    )
    page_data = await message_service.get_list(db=db, params=params)
    return response_base.success(data=page_data)


@router.get('/{pk}', summary='获取消息详情', dependencies=[DependsJwtAuth])
async def get_message_detail(
    db: CurrentSession,
    pk: Annotated[int, Path(description='消息 ID')],
) -> ResponseSchemaModel[GetMessageDetail]:
    """
    获取消息详情

    :param db: 数据库会话
    :param pk: 消息 ID
    :return:
    """
    message = await message_service.get(db=db, pk=pk)
    return response_base.success(data=message)


@router.post('', summary='创建消息', dependencies=[DependsJwtAuth])
async def create_message(
    request: Request,
    db: CurrentSessionTransaction,
    obj: CreateMessageParam,
) -> ResponseModel:
    """
    创建消息

    :param request: FastAPI 请求对象
    :param db: 数据库事务会话
    :param obj: 创建参数
    :return:
    """
    await message_service.create(db=db, obj=obj, sender_id=request.user.id)
    return response_base.success()


@router.put('/{pk}', summary='更新消息', dependencies=[DependsJwtAuth])
async def update_message(
    db: CurrentSessionTransaction,
    pk: Annotated[int, Path(description='消息 ID')],
    obj: UpdateMessageParam,
) -> ResponseModel:
    """
    更新消息

    :param db: 数据库事务会话
    :param pk: 消息 ID
    :param obj: 更新参数
    :return:
    """
    count = await message_service.update(db=db, pk=pk, obj=obj)
    if count > 0:
        return response_base.success()
    return response_base.fail()


@router.delete('', summary='批量删除消息', dependencies=[DependsJwtAuth])
async def delete_messages(
    db: CurrentSessionTransaction,
    obj: DeleteMessageParam,
) -> ResponseModel:
    """
    批量删除消息

    :param db: 数据库事务会话
    :param obj: 删除参数
    :return:
    """
    count = await message_service.delete_batch(db=db, obj=obj)
    if count > 0:
        return response_base.success()
    return response_base.fail()
