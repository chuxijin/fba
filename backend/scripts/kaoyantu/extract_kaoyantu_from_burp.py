...
def normalize_question_type(question: dict[str, Any], chapter_names: list[str] | None = None) -> str:
    ...
    if answer and re.fullmatch(r'[A-Za-z\s,，、;；]+', answer):
        letters = re.findall(r'[A-Z]', answer.upper())
        if len(letters) > 1:
            return 'multiple_choice'
        return 'single_choice'

    # 答案不是字母组合（如长文本），识别为简答题
    if answer:
        return 'short_answer'
...
def build_excel_row(
    ...
    if mapped_question_type and answer_text and re.fullmatch(r'[A-Za-z\s,，、;；]+', answer_text) and not has_multiple_option_answer(question):
...
def write_excel(
    ...
    """
    写入 V2 格式 Excel（题目表 + 材料表两个 sheet）
