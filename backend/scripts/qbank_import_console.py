...
async def run_question_import_core(args: SimpleNamespace) -> int:
    ...
    async with async_db_session() as db:
        ...
        chapter_bank_stmt = select(QuestionBank).where(QuestionBank.code == args.chapter_bank_code)
        chapter_bank = (await db.execute(chapter_bank_stmt)).scalars().first()
        if chapter_bank is None:
            raise ImportErrorBase(f"chapter bank missing: {args.chapter_bank_code}")

        chapter_stmt = select(QuestionChapter).where(QuestionChapter.bank_id == chapter_bank.id)
        chapter_rows = (await db.execute(chapter_stmt)).scalars().all()
        ...
        for chapter_name in CANONICAL_CHAPTERS:
            if chapter_name not in chapter_map:
                raise ImportErrorBase(f"chapter missing under {args.chapter_bank_code}: {chapter_name}")
        print(f"[DB] chapters_ok count={len(chapter_map)}")
                ...
                sa_text(
                    ...
                    select
                        ...
                        (
                            ...
                            join study_question_bank b on b.id = c.bank_id
                            where p.bank_id = :bank_id and b.code <> 'BANK_XINGCE'
                        ) as bad_chapter
                    """
                ),
                {"bank_id": bank_id},
