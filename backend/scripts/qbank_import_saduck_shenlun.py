...
async def import_one_paper(
    *,
    ...
    if not args.update_existing and int(existing_count or 0) > 0:
        print(f'[PAPER] {paper_index}/{total_papers} code={paper_code} 已存在题目 {int(existing_count or 0)}，跳过')
        stats.papers_skipped += 1
        return

    print(f'[PAPER] {paper_index}/{total_papers} code={paper_code} parent={parent_bank.code} name={paper.test_name}')
    ...
    placement_rows = (
        (await db.execute(select(QuestionPlacement).where(QuestionPlacement.bank_id == paper_bank.id))).scalars().all()
    )
    ...
    if question_id_list:
        existing_rows = (await db.execute(select(Question).where(Question.id.in_(question_id_list)))).scalars().all()
    ...
    print(f'[PAPER_DONE] code={paper_code} questions={len(question_ids)} materials={len(material_ids)}')
