...
async def main() -> None:
    ...
    success, error = await send_wecom_app(
        title='系统监控月报', content=markdown_content, options={'msgtype': 'markdown'}
    )
    ...
    if not success:
        print(
            f"提示说明: 按钮卡片发送失败是符合预期的，报错通常为 'require agent with callback url'，表示自建应用必须要先在微信后台成功配置保存回调 URL 后才能使用本功能。"
        )
