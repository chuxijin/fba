...
class CeleryManager:
    ...
    def start_process(self, name: str, cmd: list[str]) -> subprocess.Popen | None:
        ...
        try:
            ...
            # Windows下创建新的进程组
            if os.name == 'nt':
                process = subprocess.Popen(cmd, cwd=self.base_dir, creationflags=subprocess.CREATE_NEW_PROCESS_GROUP)
            else:
                process = subprocess.Popen(cmd, cwd=self.base_dir)

            self.processes.append({'name': name, 'process': process, 'cmd': cmd})
    ...
    def start_all_services(self):
        ...
        worker_cmd = [
            ...
            'info',
            '-P',
            'gevent' if os.name == 'nt' else 'prefork',
            '-c',
            '100',
        ]
        ...
        # 启动Beat
        beat_cmd = [sys.executable, '-m', 'celery', '-A', 'app.task.celery', 'beat', '-l', 'info']
