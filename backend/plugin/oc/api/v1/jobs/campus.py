...
from backend.plugin.oc.service.campus_recruit_service import campus_recruit_service
...
) -> ResponseSchemaModel[GetCampusRecruitDetail]:
    """获取校招岗位详情"""
    data = await campus_recruit_service.get(db=db, job_id=job_id)
    return response_base.success(data=data)


@router.get('', summary='获取校招岗位列表', dependencies=[DependsJwtAuth, DependsPagination])
async def get_campus_recruit_list(
    db: CurrentSession,
...
) -> ResponseSchemaModel[PageData[GetCampusRecruitDetail]]:
    """获取校招岗位列表"""
    data = await campus_recruit_service.get_list(
        db=db,
...
@router.put('/{job_id}', summary='更新校招岗位', dependencies=[DependsJwtAuth])
async def update_campus_recruit(
    db: CurrentSessionTransaction,
    job_id: Annotated[int, Path(description='岗位 ID')],
    obj: UpdateCampusRecruitParam,
) -> ResponseModel:
    """更新校招岗位"""
    count = await campus_recruit_service.update(db=db, job_id=job_id, obj=obj)
    if count > 0:
        return response_base.success()
    return response_base.fail()


@router.delete('/{job_id}', summary='删除校招岗位', dependencies=[DependsJwtAuth])
async def delete_campus_recruit(
    db: CurrentSessionTransaction, job_id: Annotated[int, Path(description='岗位 ID')]
) -> ResponseModel:
    """删除校招岗位"""
    count = await campus_recruit_service.delete(db=db, job_ids=[job_id])
    if count > 0:
        return response_base.success()
    return response_base.fail()
