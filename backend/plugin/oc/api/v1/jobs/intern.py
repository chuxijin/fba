...
@router.get('/stats', summary='获取实习岗位统计数据', dependencies=[DependsJwtAuth])
async def get_intern_recruit_stats(db: CurrentSession) -> ResponseModel:
    """获取实习岗位统计数据"""
    data = await intern_recruit_service.get_stats(db=db)
