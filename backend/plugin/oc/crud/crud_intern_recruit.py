...
class CRUDInternRecruit(CRUDPlus[InternRecruit]):
    """实习岗位数据库操作类"""

    async def get(self, db: AsyncSession, job_id: int) -> InternRecruit | None:
        """
        获取实习岗位详情
    ...
    async def get_select(
        ...
        return select_stmt.order_by(desc(InternRecruit.update_time), desc(InternRecruit.created_time))

    async def get_all(self, db: AsyncSession) -> Sequence[InternRecruit]:
        """
        获取所有实习岗位
    ...
    async def create(self, db: AsyncSession, obj: CreateInternRecruitParam) -> None:
        """
        创建实习岗位
    ...
    async def update(self, db: AsyncSession, job_id: int, obj: UpdateInternRecruitParam) -> int:
        """
        更新实习岗位
...
intern_recruit_dao: CRUDInternRecruit = CRUDInternRecruit(InternRecruit)
