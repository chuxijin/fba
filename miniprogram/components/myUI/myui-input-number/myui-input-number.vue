<template>
	<view class="myui-input__number">
		<view class="myui-number__minus" :class="[disabled || min >= inputValue ? 'myui-number__disabled' : '']"
			@tap="minus" :style="{ minHeight:getMinHeight }">
			<view class="myui-minus__sign" :style="{backgroundColor:signColor,width:signWidth+'rpx'}" v-if="!custom">
			</view>
			<slot></slot>
		</view>
		<input :type="type" v-model="inputValue" :disabled="disabled" @blur="blur" class="myui-number__input"
		...
		<view class="myui-number__plus" :style="{minWidth:signWidth+'rpx',minHeight:signWidth+'rpx'}"
			:class="[disabled || inputValue >= max ? 'myui-number__disabled' : '']" @tap="plus">
			<view class="myui-plus__sign-col"
				:style="{height:signWidth+'rpx',backgroundColor:signColor,left:isNvue?(signWidth/2+'rpx'):'50%'}"
				v-if="!custom">
			</view>
			<view class="myui-plus__sign-row" :style="{width:signWidth+'rpx',backgroundColor:signColor}" v-if="!custom">
			</view>
			<slot name="plus"></slot>
		</view>
	</view>
