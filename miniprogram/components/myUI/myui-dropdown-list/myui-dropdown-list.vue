<template>
	<view class="myui-dropdown__list-wrap" :class="{'myui-dropdown__list-show':isShow}"
		:style="{background:maskBackground,zIndex:zIndex}" v-if="isShow || !isNvue" ref="fui_ddl_ani" @tap="close(1)">
		<view class="myui-dropdown__list" :style="{borderRadius:radius+'rpx',background:background}">
			<scroll-view :show-scrollbar="false" class="myui-ddl__scroll" :scroll-y="true" :style="getStyles">
				<!-- #ifdef APP-NVUE -->
				<view>
				<!-- #endif -->
					<slot></slot>
					<view class="myui-dropdown__list-item"
						...
						:class="{'myui-ddl__reverse':isReverse,'myui-ddl__item-line':splitLine && itemList.length-1!==index}"
						v-for="(model,index) in itemList" :key="index" @tap.stop.prevent="itemClick($event,index)">
						<view class="myui-ddl__checkbox"
							:class="{'myui-is__checkmark':isCheckMark,'myui-ddl__checkbox-color':(!checkboxColor || checkboxColor===true) && model[checkedKey] && !isCheckMark}"
							...
							v-if="isCheckbox">
							<view class="myui-ddl__checkmark"
								:style="{borderBottomColor:checkmarkColor,borderRightColor:checkmarkColor}"
								v-if="model[checkedKey]"></view>
						</view>
						<view class="myui-ddl__flex">
							<view class="myui-ddl__icon-box"
								:class="{'myui-ddl__icon-ml':!isReverse && isCheckbox,'myui-ddl__icon-mr':isReverse}"
							...
							<text class="myui-ddl__item-text"
								:class="{'myui-ddl__text-pl':!isReverse && (isCheckbox || model[srcKey]),'myui-ddl__text-pr':isReverse && (isCheckbox || model[srcKey])}"
...
<style scoped>
	...
	/* #ifndef APP-NVUE */
	.myui-dropdown__list-show {
		visibility: visible;
		opacity: 1;
	}

	/* #endif */
	.myui-dropdown__list-item {
		/* #ifndef APP-NVUE */
		width: 100%;
		display: flex;
		box-sizing: border-box;
	...
	.myui-dropdown__list-item:active {
		/* #ifdef APP-NVUE */
