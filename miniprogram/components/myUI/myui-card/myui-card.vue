<template>
	<view class="myui-card__wrap"
		:class="{'myui-card__full':full,'myui-card__border':showBorder,'myui-card__border-radius':showBorder && !isNvue && !full}"
		...
		@tap="handleClick">
		<view class="myui-card__header" v-if="tag || title || src" :class="{'myui-card__header-line':headerLine}"
			...
			<view class="myui-card__header-left">
				<image :src="src" class="myui-card__header-thumb" v-if="src"
					:style="{height:height+'rpx',width:width+'rpx',borderRadius:imageRadius}"></image>
				<text class="myui-card__header-title" :style="{fontSize:size+'rpx',color:color}"
					v-if="title">{{title}}</text>
			</view>
			<view class="myui-card__header-right" v-if="tag">
				<text :style="{fontSize:tagSize+'rpx',color:tagColor}">{{tag}}</text>
			</view>
		</view>
		<view class="myui-card__body" :class="{'myui-card__header-line':footerLine}">
			<slot></slot>
		</view>
		<view class="myui-card__footer"
...
<style scoped>
	...
	/* #ifndef APP-NVUE */
	.myui-card__border-radius {
		border-radius: 16rpx !important;
	}

	.myui-card__border::after {
		content: ' ';
		position: absolute;
		height: 200%;
		width: 200%;
