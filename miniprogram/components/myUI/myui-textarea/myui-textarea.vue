<template>
	<view class="myui-textarea__wrap"
		:class="{'myui-textarea__flex-start':flexStart,'myui-textarea__border-nvue':textareaBorder}"
		...
		<view v-if="borderTop && !textareaBorder"
			:style="{background:borderColor,left:topLeft+'rpx',right:topRight+'rpx'}" class="myui-textarea__border-top"
			:class="{'myui-textarea__background':!borderColor}">
		</view>
		<!-- #ifndef APP-NVUE -->
		<view class="myui-textarea__border" :class="{'myui-textarea__bordercolor':!borderColor}"
			:style="{borderRadius:getRadius,borderColor:borderColor}" v-if="textareaBorder"></view>
		<!-- #endif -->
		<!-- #ifdef APP-NVUE -->
		<view class="myui-textarea__required" v-if="required && !flexStart">
			<text class="myui-form__asterisk-text" :style="{color:getRequiredColor,paddingTop:'2rpx'}">*</text>
		</view>
		<text class="myui-textarea__required" :style="{color:getRequiredColor,top:requiredTop}"
			v-if="required && flexStart">*</text>
		<!-- #endif -->
		<!-- #ifndef APP-NVUE -->
		<view class="myui-textarea__required" :class="{'myui-required__flex-start':flexStart}"
			:style="{color:getRequiredColor,top:flexStart?requiredTop:'50%'}" v-if="required">*</view>
		<!-- #endif -->
		<view class="myui-textarea__label" :style="{minWidth:labelWidth+'rpx'}" v-if="label">
			<text :style="{fontSize:labelSize+'rpx',color:labelColor}">{{label}}</text>
		</view>
		<slot name="left"></slot>
		<view class="myui-textarea__flex-1">
			<textarea ref="fuiTextarea" class="myui-textarea__self"
				:class="{'myui-text__right':textRight,'myui-textarea__disabled-styl':disabled && disabledStyle}"
				:style="{height:height,minHeight:minHeight,fontSize:size+'rpx',color:color}"
				placeholder-class="myui-textarea-placeholder" :name="name" :value="val" :placeholder="val?'':placeholder"
			...
			<view class="myui-textarea__counter" v-if="isCounter">
				<text
		...
		<view v-if="borderBottom  && !textareaBorder"
			...
			class="myui-textarea__border-bottom" :class="{'myui-textarea__background':!borderColor}"></view>
	</view>
</template>
...
<style scoped>
	...
	.myui-textarea__flex-start {
		align-items: flex-start !important;
	}

	.myui-textarea__border-top {
		position: absolute;
		top: 0;
		/* #ifdef APP-NVUE */
		height: 0.5px;
	...
	/* #ifndef APP-NVUE */
	.myui-required__flex-start {
		transform: translateY(0);
	}

	/* #endif */

	.myui-textarea__label {
		padding-right: 12rpx;
		/* #ifndef APP-NVUE */
		flex-shrink: 0;
		/* #endif */
	...
	/* #ifdef MP */
	::v-deep .myui-textarea-placeholder {
		color: var(--myui-color-minor, #ccc);
		overflow: visible;
	}
