<template>
	<view class="myui-input__wrap" :class="{'myui-input__border-nvue':inputBorder}"
		...
		<view v-if="borderTop && !inputBorder" :style="{background:borderColor,left:topLeft+'rpx',right:topRight+'rpx'}"
			class="myui-input__border-top" :class="{'myui-input__background':!borderColor}">
		</view>
		<!-- #ifndef APP-NVUE -->
		<view class="myui-input__border" :class="{'myui-input__bordercolor':!borderColor}"
			:style="{borderRadius:getBorderRadius,borderColor:borderColor}" v-if="inputBorder"></view>
		<!-- #endif -->
		<!-- #ifdef APP-NVUE -->
		<view class="myui-input__required" v-if="required">
			<text :style="{color:requiredColor || dangerColor}" class="myui-form__asterisk-text">*</text>
		</view>
		<!-- #endif -->
		<!-- #ifndef APP-NVUE -->
		<view class="myui-input__required" :style="{color:requiredColor || dangerColor}" v-if="required">*</view>
		<!-- #endif -->
		<view class="myui-input__label" :style="{minWidth:labelWidth+'rpx'}" v-if="label">
			<text :style="{fontSize:getLabelSize,color:labelColor}">{{label}}</text>
		</view>
		<slot name="left"></slot>
		<!-- #ifndef APP-NVUE -->
		<input class="myui-input__self"
			:class="{'myui-input__text-right':textRight,'myui-input__disabled-styl':disabled && disabledStyle,'myui-input__disabled':disabled}"
			:style="{fontSize:getSize,color:color,textAlign:textRight?'right':textAlign}"
			placeholder-class="myui-input__placeholder" :type="type" :name="name" :value="val"
		...
		<!-- #endif -->
		<!-- #ifdef APP-NVUE -->
		<view class="myui-input__self-wrap">
			<input ref="fuiInput" class="myui-input__self" :class="{'myui-input__text-right':textRight}"
				:style="{fontSize:getSize,color:color,textAlign:textRight?'right':textAlign}"
				placeholder-class="myui-input__placeholder" :type="type" :name="name" :value="val"
			...
			<view class="myui-input__cover" v-if="disabled || readonly" @tap="fieldClickAndroid"></view>
		</view>
		<!-- #endif -->
		<view class="myui-input__clear-wrap" :style="{background:clearColor}" v-if="clearable && val != ''"
			@tap.stop="onClear">
			<view class="myui-input__clear">
				<view class="myui-input__clear-a"></view>
			</view>
			<view class="myui-input__clear">
				<view class="myui-input__clear-b"></view>
			</view>
		</view>
		<slot></slot>
		<view v-if="borderBottom  && !inputBorder"
			:style="{background:borderColor,left:bottomLeft+'rpx',right:bottomRight+'rpx'}"
			class="myui-input__border-bottom" :class="{'myui-input__background':!borderColor}"></view>
	</view>
</template>
...
<style scoped>
	...
	.myui-input__placeholder {
		/* #ifndef APP-NVUE */
		color: var(--myui-color-minor, #ccc);
		overflow: visible;
		/* #endif */
		/* #ifdef APP-NVUE */
		color: #ccc;
	...
	/* #ifdef MP */
	::v-deep .myui-input__placeholder {
		color: var(--myui-color-minor, #ccc);
		overflow: visible;
	}
	...
	.myui-input__border {
		...
		height: 200%;
		width: 200%;
		border: 1px solid var(--myui-color-border, #EEEEEE);
		transform-origin: 0 0;
		transform: scale(0.5);
		left: 0;
		top: 0;
	...
	.myui-input__bordercolor {
		border-color: var(--myui-color-border, #EEEEEE) !important;
	}

	.myui-input__background {
		background: var(--myui-color-border, #EEEEEE) !important;
	}

	/* #endif */

	.myui-input__text-left {
		text-align: left;
	}

	.myui-input__text-right {
		text-align: right;
	}

	.myui-input__text-center {
		text-align: center;
	}

	.myui-input__disabled-styl {
		opacity: .6;
	}
</style>