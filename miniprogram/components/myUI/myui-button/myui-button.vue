<template>
	<view class="myui-button__wrap"
		:class="[!getWidth || getWidth==='100%' || getWidth===true?'myui-button__flex-1':'',disabled && !disabledBackground ? 'myui-button__opacity' : '']"
		...
		<button class="myui-button" :class="[
				bold ? 'myui-text__bold' : '',
				time && (plain || type==='link') ? 'myui-button__opacity' : '',
				!background && !disabledBackground && !plain?('myui-button__'+type):'',
				!getWidth || getWidth==='100%' || getWidth===true?'myui-button__flex-1':'',
				time && !plain && type!=='link' ? 'myui-button__active' : '',
				pc && !disabled?(plain || type==='link'?'myui-button__opacity-pc':'myui-button__active-pc'):'',
			...
			<text class="myui-button__text"
				:class="{'myui-btn__gray-color':!background && !disabledBackground && !plain && type==='gray' && color==='#fff','myui-text__bold':bold}"
				v-if="text" :style="{fontSize: getSize,lineHeight:getSize,color:getColor}">{{text}}</text>
			<slot></slot>
		</button>
		<!-- #ifndef APP-NVUE -->
		<view v-if="borderColor" class="myui-button__thin-border"
			:class="[time && (plain || type==='link') && !disabled ? 'myui-button__opacity' : '',disabled && !disabledBackground ? 'myui-button__opacity' : '']"
...
<style scoped>
	...
	/* #ifdef H5 */
	.myui-button__active-pc {
		position: relative;
	}

	.myui-button__opacity-pc:active {
		opacity: 0.5;
	}

	.myui-button__active-pc:active::after {
		content: ' ';
		background-color: var(--myui-bg-color-hover, rgba(0, 0, 0, 0.2));
		position: absolute;
		width: 100%;
		height: 100%;
		left: 0;
	...
	/* #ifndef APP-NVUE */
	.myui-button__primary {
		border-color: var(--myui-color-primary, #465CFF) !important;
		background: var(--myui-color-primary, #465CFF) !important;
	}


	.myui-button__success {
		border-color: var(--myui-color-success, #09BE4F) !important;
		background: var(--myui-color-success, #09BE4F) !important;
	}

	.myui-button__warning {
		border-color: var(--myui-color-warning, #FFB703) !important;
		background: var(--myui-color-warning, #FFB703) !important;
	}

	.myui-button__danger {
		border-color: var(--myui-color-danger, #FF2B2B) !important;
		background: var(--myui-color-danger, #FF2B2B) !important;
	}

	.myui-button__purple {
		border-color: var(--myui-color-purple, #6831FF) !important;
		background: var(--myui-color-purple, #6831FF) !important;
	}

	.myui-button__gray {
		border-color: var(--myui-bg-color-content, #F8F8F8) !important;
		background: var(--myui-bg-color-content, #F8F8F8) !important;
		color: var(--myui-color-primary, #465CFF) !important;
	}

	.myui-btn__gray-color {
		color: var(--myui-color-primary, #465CFF) !important;
	}
