<template>
	<view class="myui-popup__animation" :class="[ani.in]" :style="transformStyles" @tap="change" v-if="isShow"
		ref="fui_ani">
		<slot></slot>
	</view>
</template>
		...
		methods: {
			...
			open() {
				...
				for (let i in this.getTranfrom(false)) {
					if (i === 'opacity') {
						this.ani.in = 'myui-popup__fade-out';
					} else {
			...
			_animation(type) {
				...
				for (let i in styles) {
					if (i === 'opacity') {
						this.ani.in = `myui-popup__fade-${type ? 'in' : 'out'}`;
					} else {
