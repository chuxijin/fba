<template>
	<view class="myui-segmented__control" :class="{'myui-segmented__disabled':disabled}"
		:style="{marginTop:marginTop+'rpx',marginBottom:marginBottom+'rpx'}">
		<view class="myui-segmented__item" v-for="(item,index) in vals" :key="index"
			...
			:class="{'myui-segmented__first':index===0 && type==='button','myui-seg__item-bg':currentIndex===index && !color && type==='button','myui-seg__item-border':!color && type==='button','myui-segmented__item-border':type==='button'}"
			@click="handleClick(index)">
			<text
				:style="{fontSize:size+'rpx',color:currentIndex===index?activeColor:getColor,fontWeight:bold && currentIndex===index ?600:400}"
				:class="{'myui-segmented__disabled':item.disabled,'myui-seg__text-color':!color && currentIndex!==index}">{{item.name}}</text>
			<view class="myui-segmented__item-line" v-if="currentIndex===index && type==='text'"
