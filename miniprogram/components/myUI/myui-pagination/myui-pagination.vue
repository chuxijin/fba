<template>
	<view class="myui-pagination__wrap">
		<view class="myui-pagination__btn"
			:class="{'myui-pagination__disabled':currentIndex === 1,'myui-pagination__btn-ac':currentIndex !== 1 && highlight,'myui-pagination__color':!color}"
			:style="{width:width+'rpx',height:height+'rpx',borderColor:borderColor,background:background,borderRadius:radius+'rpx'}"
			@click="clickPrev">
			<text :class="{'myui-pagination__color':!color}" :style="{color:color,fontSize:size+'rpx'}"
				v-if="!custom">{{prevText}}</text>
			<slot name="prev"></slot>
		</view>
		<view class="myui-pagination__num" v-if="isPage && pageType==1">
			<text :class="{'myui-pagination__active-color':!currentColor}"
				:style="{color:getCurrentColor,fontSize:pageFontSize+'rpx'}">{{currentIndex}}</text>
			<text :class="{'myui-pagination__color':!pageColor}"
				:style="{color:pageColor,fontSize:pageFontSize+'rpx'}">/{{maxPage || 0}}</text>
		</view>

		<view class="myui-page__number" v-if="isPage && pageType==2">
			<view class="myui-page__num-item"
				:class="{'myui-pagination__bg':!activeBgColor && currentIndex===item,'myui-page__num-width':item==='...' || (item!='...' && item<1000),'myui-page__num-padding':item!='...' && item>999}"
				...
				<text class="myui-page__num-text" :class="{'myui-pagination__color':!color && currentIndex!==item}"
					:style="{color:currentIndex===item?activeColor:pageColor}">{{item}}</text>
			</view>
		</view>

		<view class="myui-pagination__btn"
			:class="{'myui-pagination__disabled':currentIndex === maxPage,'myui-pagination__btn-ac':currentIndex !== maxPage && highlight,'myui-pagination__color':!color}"
			:style="{width:width+'rpx',height:height+'rpx',borderColor:borderColor,background:background,borderRadius:radius+'rpx'}"
			@click="clickNext">
			<text :class="{'myui-pagination__color':!color }" :style="{color:color,fontSize:size+'rpx'}"
...
<style scoped>
	...
	.myui-pagination__btn-ac:active {
		opacity: 0.5;
	}

	.myui-pagination__num {
		/* #ifndef APP-NVUE */
		display: flex;
		/* #endif */
		flex: 1;
	...
	.myui-pagination__color {
		color: var(--myui-color-section, #333) !important;
	}

	/* #endif */

	.myui-pagination__disabled {
		opacity: 0.3;
		/* #ifdef H5 */
		cursor: not-allowed;
		/* #endif */
	...
	/* #ifdef APP-NVUE */
	.myui-page__num-width {
		width: 60rpx;
	}

	.myui-page__num-padding {
		padding: 0 8rpx;
	}

	/* #endif */

	.myui-page__num-item:active {
		opacity: .5;
	}
