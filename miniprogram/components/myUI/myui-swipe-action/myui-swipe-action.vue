<template>
	...
	<view class="myui-swipe__action-wrap" :style="{marginTop:marginTop+'rpx',marginBottom:marginBottom+'rpx'}">
		<view class="myui-swipe__action-inner" :show="isShow" :change:show="handler.showChange" :threshold="thresholdVal"
			...
			<view class="myui-swipe__action-left">
				<slot></slot>
			</view>
			<view class="myui-swipe__action-right">
				<slot name="buttons">
					<view class="myui-swipe__action-btn" :style="{background:item.background}"
						...
						<text class="myui-swipe__action-text"
	...
	<view class="myui-swipe__action-wrap" :style="{marginTop:marginTop+'rpx',marginBottom:marginBottom+'rpx'}"
		...
		<view class="myui-swipe__action-right" ref="fui_swipea_buttons">
			<slot name="buttons">
				<view class="myui-swipe__action-btn" :style="{background:item.background}"
		...
		<view class="myui-swipe__action-left" ref="fui_swipea_content">
			<slot></slot>
		</view>
	</view>
	<!-- #endif -->
	...
	<view class="myui-swipe__action-wrap" :style="{marginTop:marginTop+'rpx',marginBottom:marginBottom+'rpx'}">
		<view class="myui-swipe__action-inner" @touchstart="touchstart" @touchmove="touchmove" @touchend="touchend"
			:style="{transform:moveLeft}" :class="{'myui-swipe__action-ani':ani}">
			<view class="myui-swipe__action-left">
				<slot></slot>
			</view>
			<view class="myui-swipe__action-right" :class="[elClass]">
				<slot name="buttons">
					<view class="myui-swipe__action-btn" :style="{background:item.background}"
						v-for="(item,index) in buttons" :key="index" @touchstart="appTouchStart"
...
<script>
	...
	export default {
		...
		methods: {
			...
			//获取父元素实例
			getParent(name = 'myui-swipeaction-group') {
...
<style scoped>
	.myui-swipe__action-wrap {
		position: relative;
		overflow: hidden;
	}

	.myui-swipe__action-inner {
		/* #ifndef APP-NVUE */
		display: flex;
		flex-shrink: 0;
		/* #endif */
	...
	.myui-swipe__action-text {
		/* #ifndef APP-NVUE */
		flex-shrink: 0;
		/* #endif */
	}

	.myui-swipe__action-ani {
