<template>
	<view class="myui-radio__input"
		:class="{'myui-radio__disabled':disabled,'myui-radio__color':!getColor && val && !isCheckMark}"
		...
		<view class="myui-check__mark" :style="{borderBottomColor:checkMarkColor,borderRightColor:checkMarkColor}"
			v-if="val"></view>
		<radio class="myui-radio__hidden" style="opacity: 0;position: absolute;" :color="getColor" :disabled="disabled" :value="value" :checked="val"></radio>
	</view>
</template>
...
<script>
	export default {
		...
		methods: {
			...
			getParent(name = 'myui-radio-group') {
