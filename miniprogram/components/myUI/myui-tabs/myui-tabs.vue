<template>
	<scroll-view class="myui-tabs__scrollbox"
		:class="{'myui-tabs__fixed':isFixed && !isSticky,'myui-tabs__sticky':isSticky}" :scroll-with-animation="true"
		...
		<view class="myui-scroll__view" :class="{'myui-tabs__full':!alignLeft}">
			<view v-for="(tab, index) in vals" :key="index" class="myui-tabs__item"
				:class="{'myui-tabs__full':!alignLeft}"
				:style="{paddingLeft:itemPadding+'rpx',paddingRight:itemPadding+'rpx'}" :id="tab.fui_s_id"
				@tap="switchTab(index)">
				<view class="myui-tabs__text-wrap"
					:class="{'myui-tabs__wrap-disabled':tab[disabledKey],'myui-tabs__item-column':direction==='column' && tab.icon}"
					:style="{height:height+'rpx'}">
					<view class="myui-tabs__line-wrap" :class="{'myui-tabs__line-center':center}"
						:style="{bottom:bottom +'rpx',left:`-${padding}rpx`,right:`-${padding}rpx`}" v-if="isSlider">
						<view class="myui-tabs__ac-line"
							:class="{'myui-tabs__line-short':short,'myui-tabs__full':!short,'myui-tabs__slider-color':!getSliderBgColor}"
					...
					<image class="myui-tabs__icon" :class="{'myui-tabs__icon-column':direction==='column'}"
						:src="tabIndex===index && tab.selectedIcon?tab.selectedIcon:tab.icon" v-if="tab.icon">
					</image>
					<!-- #ifdef APP-NVUE -->
					<view class="myui-tabs__text">
						<text
						...
						<text class="myui-tabs__text-nvue"
							:class="{'myui-tabs__selected-color':!getSelectedColor && tabIndex===index,'myui-tabs__text-color':!color && tabIndex!==index}"
						...
						<text
							:class="{'myui-tabs__badge-color':!getBadgeBgColor,'myui-tabs__badge-dot':isDot,'myui-tabs__badge':!isDot}"
...
<style scoped>
	...
	.myui-tabs__item-column {
		flex-direction: column !important;
	}

	.myui-tabs__icon-column {
	...
	.myui-tabs__line-short {
		width: 45rpx !important;
	}

	/* #ifndef APP-NVUE */
	.myui-tabs__selected-color {
		color: var(--myui-color-primary, #465CFF) !important;
	}

	.myui-tabs__text-color {
		color: var(--myui-color-subtitle, #7F7F7F) !important;
	}

	.myui-tabs__slider-color {
		background: var(--myui-color-primary, #465CFF) !important;
	}

	.myui-tabs__badge-color {
