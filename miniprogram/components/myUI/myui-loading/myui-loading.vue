<template>
	<view :class="{'myui-loading__mask':(isMask || isNvue) && isFixed}"
		:style="{backgroundColor:isFixed?maskBgColor:'transparent'}">
		<view class="myui-loading myui-loading__wrap" :class="{'myui-loading__fixed':isFixed && !isNvue}"
			:style="{ backgroundColor: backgroundColor,position:isFixed && !isNvue?'fixed':'static' }"
			v-if="type === 'col'">
			<image ref="fui_loading" class="myui-loading__ani" :src="srcCol"></image>
			<text class="myui-loading__text"
				:style="{color:colColor,fontSize:size+'rpx','line-height':size+'rpx'}">{{ text }}</text>
		</view>
		<view v-else class="myui-loading myui-loading__row" :class="{'myui-loading__fixed':isFixed && !isNvue}"
			:style="{position:isFixed && !isNvue?'fixed':'static'}">
			<!-- <loading-indicator :animating="true"></loading-indicator> -->
			<image ref="fui_loading" class="myui-loading-row__ani" :src="srcRow"></image>
			<text class="myui-loading__text"
