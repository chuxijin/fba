<template>
	<view class="myui-footer" :class="[isFixed?'myui-footer__fixed-bottom':'']"
		:style="{background:background,bottom:bottom+'rpx'}">
		<view class="myui-footer__link" v-if="navigate.length>0">
			<navigator v-for="(item,index) in navigate" :key="index" class="myui-link__item" hover-class="myui-link-hover"
				hover-stop-propagation :open-type="item.openType || 'navigate'" :url="item.url" :delta="item.delta">
				<text class="myui-link__text"
					:class="{'myui-link__color':!item.color,'myui-link__text-border':index===navigate.length-1}"
		...
		<view class="myui-footer__text"
			:class="{'myui-as__safe-weex':iphoneX && safeArea,'myui-footer__safearea':safeArea}">
...
<style scoped>
	...
	/* #ifndef APP-NVUE */
	.myui-link__color {
		color: var(--myui-color-link, #465CFF) !important;
	}
	...
	.myui-link__text-border {
		border-right-width: 0;
	}


	/* #ifndef APP-NVUE */
	.myui-link__item::before {
		content: " ";
		position: absolute;
		right: 0;
		top: 4rpx;
		width: 1px;
		bottom: 4rpx;
		border-right: 1px solid var(--myui-color-label, #B2B2B2);
	...
	.myui-link__item:last-child::before {
