<template>
	<view class="myui-loadmore__wrap" :class="['myui-loadmore__'+direction]" :style="{height:height+'rpx'}">
		<view class="myui-loadmore__icon" ref="fui_loadmore"
			:class="{'myui-loadmore__border-left':!isNvue && !activeColor}"
		...
		<image class="myui-loadmore__icon-ani" ref="fui_loadmore" :src="src"
			:style="{width:iconWidth+'rpx',height:iconWidth+'rpx'}" v-if="src && state==2"></image>
		<text :class="{'myui-loadmore__text':direction==='col'}"
...
<style scoped>
	...
	.myui-loadmore__icon {
		...
		/* #ifndef APP-NVUE */
		border-radius: 50%;
		animation: myui-rotate 0.7s linear infinite;
		/* #endif */
	}

	.myui-loadmore__icon-ani {
		margin: 0 6px;
		/* #ifndef APP-NVUE */
		display: block;
		animation: myui-rotate 0.7s linear infinite;
	...
	/* #ifndef APP-NVUE */
	.myui-loadmore__border-left {
		border-left-color: var(--myui-color-primary, #465CFF) !important;
	}

	@-webkit-keyframes myui-rotate {
		0% {
			transform: rotate(0);
		}
