<template>
	<view class="myui-dropdown__menu" ref="fui_dm_wrap" @tap="resetCalc">
		<slot></slot>
		<view class="myui-ddm__mask" :style="{background:maskBackground}" v-if="isShow && isMask" @tap="close(1)">
		</view>
		<view class="myui-dropdown__menu-list"
			:class="{'myui-ddm__down':direction!=='up','myui-ddm__up':direction==='up','myui-ddm__down-show':isShow && direction!=='up','myui-ddm__up-show':isShow && direction==='up'}"
			:style="getStyles" v-if="isNvueShow || !isNvue" ref="fui_ddm_list">
			<scroll-view :show-scrollbar="false" class="myui-ddm__scroll" scroll-y
				:style="{height:isNvue?maxHeight+'rpx':'auto', maxHeight:maxHeight+'rpx',minWidth:minWidth+'rpx'}">
				<view>
					<slot name="item"></slot>
					<view class="myui-dropdown__menu-item"
						:style="{background:background,padding:padding,borderBottomColor:splitLine && isNvue?lineColor:'transparent'}"
						:class="{'myui-ddm__reverse':isReverse,'myui-ddm__item-line':splitLine && itemList.length-1!==index}"
						v-for="(model,index) in itemList" :key="index" @tap.stop="itemClick(index)">
						<view class="myui-ddm__checkbox"
							:class="{'myui-is__checkmark':isCheckMark,'myui-ddm__checkbox-color':(!checkboxColor || checkboxColor===true) && model[checkedKey] && !isCheckMark}"
							...
							v-if="isCheckbox">
							<view class="myui-ddm__checkmark"
								:style="{borderBottomColor:checkmarkColor,borderRightColor:checkmarkColor}"
								v-if="model[checkedKey]"></view>
						</view>
						<view class="myui-ddm__flex">
							<view class="myui-ddm__icon-box"
								:class="{'myui-ddm__icon-ml':!isReverse && isCheckbox,'myui-ddm__icon-mr':isReverse}"
							...
							<text class="myui-ddm__item-text"
								:class="{'myui-ddm__text-pl':!isReverse && (isCheckbox || model[srcKey]),'myui-ddm__text-pr':isReverse && (isCheckbox || model[srcKey])}"
...
<style scoped>
	...
	/* #ifndef APP-NVUE */
	.myui-ddm__item-line::after {
		content: '';
		position: absolute;
		border-bottom: 1px solid var(--myui-color-border, #EEEEEE);
		/* #ifdef H5 */
	...
	.myui-dropdown__menu-item:active {
		/* #ifdef APP-NVUE */
		background-color: rgba(0, 0, 0, .2) !important;
		/* #endif */
		/* #ifndef APP-NVUE */
		background-color: var(--myui-bg-color-hover, rgba(0, 0, 0, .2)) !important;
		/* #endif */
	}
