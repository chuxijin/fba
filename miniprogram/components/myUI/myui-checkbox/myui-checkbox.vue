<template>
	<view class="myui-checkbox__input"
		:class="{'myui-checkbox__disabled':disabled,'myui-checkbox__color':!getColor && val && !isCheckMark}"
		...
		<view class="myui-check__mark" :style="{borderBottomColor:checkMarkColor,borderRightColor:checkMarkColor}"
			v-if="val"></view>
		<checkbox class="myui-checkbox__hidden" style="opacity: 0;position: absolute;" :color="getColor"
...
<script>
	export default {
		...
		created() {
			...
			this.label = this.getParent('myui-label')
			if (this.label) {
		...
		methods: {
			...
			getParent(name = 'myui-checkbox-group') {
