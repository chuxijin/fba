    ...
    "baseKeypointVOS": [
        ...
        {"id": 783918, "name": "数量关系", "children": [
            {"id": 784242, "name": "数学运算", "children": [
                ...
                {"id": 784245, "name": "年龄问题"}, {"id": 784246, "name": "周期问题", "children": [
...
def gen_inserts():
    """Generate SQL inserts grouped by depth."""
    all_nodes = {}
    for idx, d1 in enumerate(api_data["baseKeypointVOS"]):
        node = {"id": d1["id"], "name": d1["name"], "depth": 0, "parent_id": None, "sort_order": idx + 1}
        all_nodes[d1["id"]] = node
        if "children" in d1:
            for idx2, d2 in enumerate(d1["children"]):
                node2 = {"id": d2["id"], "name": d2["name"], "depth": 1, "parent_id": d1["id"], "sort_order": idx2 + 1}
                all_nodes[d2["id"]] = node2
                if "children" in d2:
                    for idx3, d3 in enumerate(d2["children"]):
                        node3 = {"id": d3["id"], "name": d3["name"], "depth": 2, "parent_id": d2["id"], "sort_order": idx3 + 1}
                        all_nodes[d3["id"]] = node3
                        if "children" in d3:
                            for idx4, d4 in enumerate(d3["children"]):
                                node4 = {"id": d4["id"], "name": d4["name"], "depth": 3, "parent_id": d3["id"], "sort_order": idx4 + 1}
                                all_nodes[d4["id"]] = node4

    for depth in range(4):
        nodes = [n for n in all_nodes.values() if n["depth"] == depth]
        if not nodes:
            continue
        lines = []
        for n in nodes:
            ...
            name = esc(n["name"])
            if n["parent_id"]:
                pid = f"(SELECT id FROM qbank_v2_knowledge_point WHERE code = 'fenbi_{n['parent_id']}' AND system_id = (SELECT id FROM qbank_v2_knowledge_system WHERE code = 'xingce'))"
            else:
                pid = "NULL"
            lines.append(f"SELECT (SELECT id FROM qbank_v2_knowledge_system WHERE code = 'xingce'), '{code}', '{name}', {depth}, {pid}, '/{code}', {n['sort_order']}, 1, NOW();")
        
        filename = f"scripts/insert_depth_{depth}.sql"
        with open(filename, "w", encoding="utf-8") as f:
            f.write("INSERT INTO qbank_v2_knowledge_point (system_id, code, name, depth, parent_id, path, sort_order, created_by, created_time)\n")
            f.write("\n".join(lines))
        
        print(f"Generated {filename}: {len(nodes)} inserts")
