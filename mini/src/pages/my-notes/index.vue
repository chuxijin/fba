...
async function loadData() {
  ...
  loading.value = true
  try {
    const data = await fbaApi.qbank.note.getStatistics(groupMode.value) as any
    statistics.value = {
    ...
    <view class="mt-4 px-4 pb-24">
      ...
      <view class="mb-4 flex items-center gap-0 rounded-full bg-white/80 p-1 shadow-sm backdrop-blur-md">
        <view
          ...
          :class="groupMode === m.key ? 'bg-[#2563EB] text-white shadow-sm' : 'text-[#64748B]'"
      ...
      <view v-else-if="flatGroups.length > 0" class="border border-white/60 rounded-2xl bg-white/85 shadow-[0_2px_12px_-6px_rgba(0,0,0,0.06)] backdrop-blur-md overflow-hidden">
        ...
        >
          <view class="flex items-center gap-2.5 min-w-0 flex-1">
            ...
            <view v-else class="h-6 w-6 flex shrink-0 items-center justify-center rounded-md" :class="groupMode === 'bank' ? 'bg-[#EFF6FF]' : 'bg-[#DBEAFE]'">
              <view :class="groupMode === 'bank' ? 'i-carbon-document text-[13px] text-[#3B82F6]' : 'i-carbon-notebook text-[13px] text-[#2563EB]'" />
            </view>
