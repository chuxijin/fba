...
async function loadData(mode: GroupMode = groupMode.value) {
  ...
  try {
    const data = await fbaApi.qbank.favorite.getStatistics(mode) as any
...
function getModeTotalCount(mode: GroupMode) {
  return Number(getModeStatistics(mode)?.total_count || 0)
}
  ...
  <view class="relative min-h-screen from-[#FFFBEB] via-[#FFFDF8] to-[#FAFAFA] bg-gradient-to-b text-[#334155]">
    ...
    <view class="mt-4 px-4 pb-24">
      ...
      <view class="mb-5 border border-white/60 rounded-2xl bg-white/85 p-5 shadow-[0_4px_24px_-10px_rgba(0,0,0,0.06)] backdrop-blur-md">
        <view class="grid grid-cols-2 gap-3">
          <view class="flex flex-col items-center rounded-xl bg-[#FFFBEB] py-3">
            <text class="text-[22px] text-[#D97706] font-black">{{ getModeStatistics(groupMode).total_count }}</text>
            <text class="mt-1 text-[11px] text-[#94A3B8]">总收藏</text>
          </view>
          <view class="flex flex-col items-center rounded-xl bg-[#F8FAFC] py-3">
            <text class="text-[22px] text-[#475569] font-black">{{ getModeStatistics(groupMode).folder_count || 0 }}</text>
      ...
      <view class="mb-3">
        ...
        >
          <view
            class="absolute bottom-1.5 top-1.5 rounded-[14px] bg-[#D97706] shadow-sm transition-all duration-300"
            :style="{
              width: 'calc(50% - 6px)',
            ...
            >
              ...
              >
                <view class="flex items-center gap-2.5 min-w-0 flex-1">
                  ...
                  <view v-else class="h-6 w-6 flex shrink-0 items-center justify-center rounded-md" :class="modeItem.key === 'bank' ? 'bg-[#EFF6FF]' : 'bg-[#FEF3C7]'">
                    <view :class="modeItem.key === 'bank' ? 'i-carbon-document text-[13px] text-[#3B82F6]' : 'i-carbon-star-filled text-[13px] text-[#F59E0B]'" />
                  </view>
