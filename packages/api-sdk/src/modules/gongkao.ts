...
export function createGongkaoModule(client: ApiClient): GongkaoModule {
  ...
  return {
    ...
    content: {
      getTags(limit = 50) { return client.get<string[]>('/content/tags', { params: { limit } }); },
      getDetail(id) { return client.get<GkContentResult>(`/content/${id}`); },
      getBySlug(slug) { return client.get<GkContentResult>(`/content/slug/${slug}`); },
      getList(params) { return client.get<PageData<GkContentResult>>('/content/list', { params: params as Record<string, unknown> }); },
      create(data) { return client.post('/content', data); },
      update(id, data) { return client.put(`/content/${id}`, data); },
      remove(ids) { return client.delete('/content', { params: { pk: ids } }); },
      incrementView(id) { return client.post(`/content/${id}/view`); },
    },
    gangwei: {
